-- Record paid source-month markers so all profiles hide already collected debt.
...
with payment_sources as (
  select
    n.actor_member_id as settled_by_member_id,
...
valid_sources as (
  select
    ...
    case when profile_id_text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
      then profile_id_text::uuid
      else null
    end as profile_id,
    month
  from payment_sources
  where group_id_text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
...
resolved_sources as (
  select
    ...
    valid_sources.month,
    valid_sources.settled_by_member_id
  from valid_sources
  left join lateral (
    select id
    from public.members
