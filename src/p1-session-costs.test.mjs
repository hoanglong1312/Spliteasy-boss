...
test('calendar detail panel contains the P1 session-cost editor contract', () => {
  ...
  assert.match(calendarSource, /const removeExtra = \(id\) => \{/)
  assert.match(calendarSource, /removeExtra\(extra\.id\)/)
  assert.match(calendarSource, />Xóa<\/button>/)
...
test('batch entry sends batch water saves and can cancel back', () => {
  ...
  assert.match(batchSource, /parsed \? parsed\.waterAmount : parseAmount\(session\.water \|\| session\.waterAmount \|\| 0\)/)
  assert.match(batchSource, /position:\s*'sticky'[\s\S]*top:\s*0[\s\S]*background:\s*colors\.pageBg[\s\S]*zIndex:\s*10/)
...
test('batch entry uses one full-width save action and no reset-zero button', () => {
  ...
  assert.match(batchSource, /batchPreviewStatus\(session, parsed\)/)
  assert.match(batchSource, /Sẽ xóa/)
