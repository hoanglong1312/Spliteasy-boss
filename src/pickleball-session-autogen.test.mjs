...
test('calendar screen moves individual tickets into the calendar flow', () => {
  ...
  assert.match(calendarSource, /const \[editingTicket, setEditingTicket\] = useState\(null\)/)
  assert.match(calendarSource, /onEdit=\{setEditingTicket\}/)
  assert.match(calendarSource, />Sửa<\/button>/)
  assert.match(calendarSource, /onAction\?\.\('updateTicket', \{ ticketId: editingTicket\.id, \.\.\.payload \}\)/)
...
test('settings save deletes scheduled sessions and regenerates when schedule weekdays change', () => {
  ...
  assert.match(legacyDeleteBlock, /\.or\('status\.is\.null,status\.eq\.scheduled'\)/)
