...
test('app-v2 handles markAttendance and validates addTicket payloads before insert', () => {
  ...
  assert.match(appSource, /const hasBillableTicketMembers = selectedTicketMemberIdsHavePerSessionMember\(state, memberIds, sessionDate\)/)
  assert.match(appSource, /if \(totalAmount <= 0 && hasBillableTicketMembers\) throw new Error\('ticket_total_amount_required'\)/)
