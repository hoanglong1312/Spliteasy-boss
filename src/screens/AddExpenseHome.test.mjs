...
test('Home hero review chip is an explicit settle-all action', () => {
  ...
  assert.match(homeSource, /borderLeft: '2px solid rgba\(52,211,153,0\.34\)'/);
  assert.match(homeSource, /paddingLeft: 10/);
  ...
  assert.match(homeSource, /const \[payForDetailsExpanded, setPayForDetailsExpanded\] = useState\(false\)/);
  ...
  assert.match(homeSource, /const selectedPayForPaymentItems = selectedPayForGroups\.flatMap\(group => group\.items\)/);
  assert.match(homeSource, /const selectedPayForTotal = selectedPayForPaymentItems\.reduce/);
  ...
  assert.match(homeSource, /const amountToPay = selectedOwnPaymentItems\.reduce/);
  assert.match(homeSource, /paymentNames\.join\(', '\)/);
  assert.match(homeSource, /<PaymentItemSection[\s\S]*title=\{`Các khoản của \$\{data\?\.memberName \|\| 'bạn'\}`\}[\s\S]*items=\{ownPaymentItems\}[\s\S]*checkedKeys=\{selectedPaymentSourceKeys\}/);
  assert.match(homeSource, /selectedPayForGroups\.map\(group =>/);
  assert.match(homeSource, /Đang thanh toán hộ/);
  assert.match(homeSource, /aria-expanded=\{payForDetailsExpanded\}/);
  assert.match(homeSource, /setPayForDetailsExpanded\(value => !value\)/);
  assert.match(homeSource, /title=\{`Chi tiết khoản của \$\{group\.row\.name\}`\}/);
