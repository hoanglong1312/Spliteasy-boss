...
test('Home hero review chip is an explicit settle-all action', () => {
  ...
  assert.match(homeSource, /const paymentPeriodLabel = paymentItemsPeriodLabel\(selectedOwnPaymentItems, data\?\.monthLabel\)/);
  ...
  assert.match(homeSource, /<PaymentItemSection[\s\S]*title=\{paymentSectionTitle\}[\s\S]*items=\{ownPaymentItems\}[\s\S]*checkedKeys=\{selectedPaymentSourceKeys\}/);
  ...
  assert.match(homeSource, /const coveredSources = \[\s*\.\.\.selectedOwnPaymentItems\.map\(paymentItemToCoveredSource\),[\s\S]*selectedPayForRows\.flatMap/);
