...
test('PickleballCalendar ticket day rows open full participant detail sheet', () => {
  ...
  assert.match(ticketDayPanel, /\{ticket\.memberIds\.length\} người đánh vé lẻ/)
  assert.match(ticketDayPanel, /lineHeight: 1\.45/)
  ...
  assert.match(detailSheet, /\{members\.length\} người đánh vé lẻ/)
