...
test('member deletion is confirmed with the shared in-app sheet before dispatch', () => {
  ...
  assert.match(memberListSource, /await onAction\?\.\('removeMemberToVanglai', \{ memberId: deleteConfirmMember\.id, groupId: data\.groupId \}\)/)
  ...
  assert.match(memberDetailSource, /await onAction\?\.\('removeMemberToVanglai', \{ memberId: d\.id, groupId: d\.groupId \}\)/)
