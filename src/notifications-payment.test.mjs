...
test('treasurer payment sheet can choose refund members and show their QR', () => {
  ...
  assert.match(homeSource, /\{selected && selectedRefund && \(/)
  assert.match(homeSource, /gridTemplateColumns: '1fr auto'/)
  assert.match(homeSource, /justifyContent: 'center', marginTop: 10/)
  assert.match(homeSource, /width: 150, height: 150/)
  assert.doesNotMatch(homeSource, /<PaymentInfoLine label="STK"/)
  assert.doesNotMatch(homeSource, /\{refundBank\.holder \|\| selectedRefund\.name\}/)
